...
class Banks extends Admin
{
    ...
    public function home(?array $data): void
    {
        ...
        $banks = (new BankCoeficient())->find()->fetch(true);
        ...
        if (!empty($data["action"]) && $data["action"] == "create") {
            ...
            if ($data["count_coeficient"] > 0) {
                for ($i = 0; $i <= $data["count_coeficient"]; $i++) {
                    ...
                    $coeficient->organ = $data["organ_$i"];
                    ...
                    $coeficient->bank_coeficient = $createBank->id;
                    $coeficient->save();
                }
            }
    ...
    public function bankUpdate(?array $data): void
    {
        ...
        if (!empty($data["action"]) && $data["action"] == "update") {
            ...
            $coeficients = (new Coeficient())->find("bank_coeficient=:b", "b={$bank_coeficiente->id}")->fetch(true);
            if($coeficients){
            ...
            if ($data["count_coeficient_old"] > 0) {
                for ($i = 1; $i <= $data["count_coeficient_old"]; $i++) {
                    ...
                    $coeficient->bank_coeficient = $bank_coeficiente->id;
                    $coeficient->save();
                }
            }
