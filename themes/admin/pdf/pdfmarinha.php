    ...
    <tr>
      <td colspan="4">
        ...
        <tbody>
          ...
          <tr>
            <td>
				<?php if ($each_contract->BANCO_EMPRES != "") : ?><?php if ($each_contract->TIPO != 'CC') : ?>EMPRESTIMO <?php endif; ?><?= $each_contract->BANCO_EMPRES ?><?php endif; ?></div>
			  </td>
            <td >
				<?php if ($each_contract->TIPO == 'CC') : ?><div><?= "R$" . $each_contract->VALOR;
				else : ?><div>&nbsp;</div><?php endif; ?></div>
			  </td>
            <td>
				<?php if ($each_contract->TIPO == 'EMP' || $each_contract->TIPO == 'ND') : ?><div><?= "R$" . $each_contract->VALOR;
				else : ?><div>&nbsp;</div><?php endif; ?>
			  </td>
            <td>
				<?php if ($each_contract->TIPO != 'CC') : ?><?= date_month_year($each_contract->PRAZO) ?><?php endif; ?></td>
          </tr>
			
<?php
			if ($each_contract->TIPO == 'CC') : $valor_receita += str_price_invert($each_contract->VALOR);
			endif;
			if ($each_contract->TIPO == 'EMP' || $each_contract->TIPO == 'ND') : $valor_desconto += str_price_invert($each_contract->VALOR);
			endif;
		endforeach;
					endif;
					?>
